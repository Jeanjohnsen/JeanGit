...
# Creates the '.jgit' directory and a subdirectory 'objects' to store all
# objects.
...
def init():
    ...
    # The resulting hash (OID) is then used to store the object in the
    # '.jgit/objects' directory.


def hash_object(data, type_='blob'):
    # Create the object content with type and data
    obj = type_.encode() + b'\x00' + data
    oid = hashlib.sha1(obj).hexdigest()  # Generate the SHA-1 hash (object ID)

    # Save the object content to a file named after its OID in the objects
    # directory
    ...
    # The object is read from the '.jgit/objects' directory and validated
...
def get_object(oid, expected='blob'):
    ...
    # If an expected type is provided, ensure the object's type matches the
    # expected type
...
def get_ref(ref):
    if os.path.isfile(f'{GIT_DIR}/{ref}', 'w'):
